...
### 4.5 守护进程运行控制、即时同步与实时日志

进入 **「概览」**（单账号看板）或 **「服务运行时」** 页面：

#### 1. 运行状态与细分活动监控
OneWeb 区分**系统服务级状态**与**业务同步活动状态**：
- **服务级状态**：
  - `运行中`（绿色徽章，伴随真实 PID 与内存占用）
  - `启动中...`（黄色徽章，对应 systemd `activating` 中间状态）
  - `已停止`（灰色徽章）
  - `异常失败`（红色徽章）
- **同步活动细分指示器（Live Activity Indicator）**：
  - **`🟢 空闲待命（已同步至最新）`**：表明当前轮次比对传输已完全搞定，记录有明确的 **「上次同步完成时间」**。守护进程正在后台通过 Linux 内核 `inotify` 监听本地与云端变动。
  - **`🔄 正在同步与比对数据...`**：表明当前正在与微软 Graph API 进行增量传输或比对。

#### 2. 生命周期与即时操作栏
- **`▶ 启动服务`**：拉起后台 `onedrive.service` 守护进程。
- **`⏹ 停止服务`**：安全退出后台守护进程。
- **`⚡ 立刻同步 (Sync Now)`**：无需等待 300 秒（5分钟）轮询周期，**立即强制触发一轮全量云端与本地文件比对与传输**。
- **`↻ 重启服务`**：重启 systemd 守护进程单元。

> [!NOTE]
> **关于为什么“同步已完成”而服务仍显示“运行中”**：
> `onedrive` 在 Linux 下作为 systemd 服务运行时采用 `--monitor` 守护进程模式，其设计目标是 7x24 小时后台值守。日志中的 `Sync complete` 代表当期数据一致，完成后进程保持存活进入“空闲待命”状态，以便在您下次保存或新增本地文件时秒级自动上传。若进程在同步完成后退出，后续的新增修改将无法被自动感知。

#### 3. 实时日志查看器 (LogViewer)
- 直通 `journalctl --user` 日志流，以每 3 秒自动轮询 + 格式化 JSON 呈现。
